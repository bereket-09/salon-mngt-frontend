...
import {
  TextField, Radio, RadioGroup, FormControlLabel, FormControl, FormLabel, Grid, Paper,
  Typography, Button, Box, TextareaAutosize, Alert
} from '@mui/material';
...
//   console.log(createdByUsername,account.username)
  ...
  const handleDateChange = async (newDate) => {
    ...
    try {
      ...
      const response = await axios.get(`http://localhost:4000/api/questions/checkQuestionCount?date=${formattedDate}`);
  ...
  const validate = () => {
    ...
    ['A', 'B', 'C'].forEach(option => {
      if (!options[option].english) newErrors[`${option}English`] = `Option ${option} in English is required`;
      if (!options[option].amharic) newErrors[`${option}Amharic`] = `Option ${option} in Amharic is required`;
    });
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:4000/api/questions', formattedData, {
        headers: {
      ...
      if (response.data.code === 1000) {
        ...
        const errorMessage = error.response?.data?.message || 'An error occurred while creating the question';
  ...
  return (
    ...
    <Box sx={{ p: 4, backgroundColor: '#f5f5f5', minHeight: '90vh', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
      <Paper elevation={6} sx={{ p: 4, maxWidth: 700, width: '100%', borderRadius: '12px' }}>
        ...
        <Grid container spacing={3}>
          <Grid item xs={12}>
            <FormLabel component="legend" sx={{ fontWeight: 'bold', mb: 1 }}>Question (English)</FormLabel>
            <TextareaAutosize
              minRows={4}
          ...
          <Grid item xs={12}>
            <FormLabel component="legend" sx={{ fontWeight: 'bold', mb: 1 }}>Question (Amharic)</FormLabel>
            <TextareaAutosize
              minRows={4}
              style={{ width: '100%', padding: '10px', borderRadius: '8px', borderColor: '#1976d2' }}
            ...
            <React.Fragment key={option}>
              <Grid item xs={6}>
                <TextField
                  ...
                  required
                />
                {errors[`${option}English`] && <Alert severity="error">{errors[`${option}English`]}</Alert>}
              </Grid>
              <Grid item xs={6}>
              <TextField
  ...
  required
/>
                {errors[`${option}Amharic`] && <Alert severity="error">{errors[`${option}Amharic`]}</Alert>}
              </Grid>
            </React.Fragment>
          ))}
          ...
          <Grid item xs={12}>
            <FormControl component="fieldset">
              <FormLabel component="legend" sx={{ fontWeight: 'bold', color: '#1976d2' }}>Select the Correct Answer</FormLabel>
              <RadioGroup
                row
                value={correctAnswer}
