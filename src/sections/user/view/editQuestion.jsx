...
import { TextField, Button, Grid, Paper, Typography, Box, TextareaAutosize, Alert, MenuItem, Select, InputLabel, FormControl, RadioGroup, FormControlLabel, Radio } from '@mui/material';
...
const EditQuestion = () => {
    ...
    const fetchQuestionDetails = async () => {
      try {
        ...
        if (response.data.code === 1000) {
          ...
          setOptions({
            A: { english: questionData.english_options[0], amharic: questionData.amharic_options[0] },
            B: { english: questionData.english_options[1], amharic: questionData.amharic_options[1] },
            C: { english: questionData.english_options[2], amharic: questionData.amharic_options[2] }
          });
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.put(`http://localhost:4000/api/questions/${id}`, formattedData, {
        headers: {
      ...
      const errorMessage = error.response?.data?.message || 'An error occurred while updating the question';
...
<Grid item xs={12}>
              <FormControl component="fieldset">
                <Typography variant="h6" gutterBottom>Select Correct Answer</Typography>
                <RadioGroup
                  value={correctAnswer}
