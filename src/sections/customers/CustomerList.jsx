...
const STATUS_CONFIG = {
  active: { label: 'WAITING', color: 'warning', icon: 'solar:clock-circle-linear' },
  checked_in: { label: 'IN SALON', color: 'success', icon: 'solar:pulse-linear' },
  completed: { label: 'COMPLETED', color: 'secondary', icon: 'solar:check-circle-linear' },
};
  ...
  if (!customers.length) {
    return (
      <Box sx={{ py: 10, textAlign: 'center', bgcolor: alpha(theme.palette.secondary.main, 0.02), borderRadius: 2.5, border: '2px dashed', borderColor: alpha(theme.palette.divider, 0.1) }}>
        <Iconify icon="solar:users-group-rounded-linear" width={64} sx={{ color: 'text.disabled', opacity: 0.1, mb: 2 }} />
  ...
  return (
    <Grid container spacing={2.5}>
      {sortedCustomers.map((customer) => {
        ...
        const status = STATUS_CONFIG[customer.status] || { label: 'UNKNOWN', color: 'default', icon: 'solar:info-circle-linear' };
              ...
              <Stack spacing={3}>
                <Stack direction="row" spacing={2} alignItems="center">
                  ...
                  <Box sx={{ flexGrow: 1, minWidth: 0 }}>
                    ...
                    <Stack direction="row" spacing={1} alignItems="center">
                      <Iconify icon="solar:phone-linear" sx={{ width: 14, color: 'text.disabled' }} />
                ...
                <Box sx={{ p: 1.5, bgcolor: alpha(theme.palette.background.neutral, 0.8), borderRadius: 2 }}>
                  <Stack direction="row" justifyContent="space-between" alignItems="center">
                    <Stack direction="row" spacing={1} alignItems="center">
                      <Iconify icon="solar:map-point-linear" sx={{ width: 14, color: 'secondary.main' }} />
